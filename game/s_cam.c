...
{
	/* This creates a tempory entity we can 
		manipulate within this function
	*/
	edict_t* chasecam;

	/* Tell everything that looks at the 
		toggle that our chasecam is on and 
		working 
	*/
	...
	/* Make out gun model "non-existent" so 
		it's more realistic to the player 
		using the chasecam 
	*/
	...
	// this turns off Quake2's inclination to 
	// predict where the camera is going,
	// making a much smoother ride
	ent->client->ps.pmove.pm_flags |= PMF_NO_PREDICTION; 

	// this line tells Quake2 not to send the 
	// unnecessary info about the camera to 
	// other players
	...
	/* Now, make the angles of the player model,
		(!NOT THE HUMAN VIEW!) be copied to 
		the same angle of the chasecam entity
	*/
	VectorCopy(ent->s.angles, chasecam->s.angles);



	/* Clear the size of the entity, so it 
		DOES technically have a size,
		but that of '0 0 0'-'0 0 0'. 
		(xyz, xyz). mins = Minimum size,
		maxs = Maximum size 
	*/
	...
	/* Make the chasecam's origin (position) 
		be the same as the player
		entity's because as the camera starts, 
		it will force itself out
...
{
	/* Keep thinking this function to 
		check all the time whether the
		player is out of the water 
	*/
    ent->nextthink = level.time + 0.100;
	
	/* If the player is dead, the 
		camera is not wanted... 
		Kill me and stop
		the function. (return;) 
	*/
	...
	/* If the player is still underwater, 
		break the routine 
	*/
